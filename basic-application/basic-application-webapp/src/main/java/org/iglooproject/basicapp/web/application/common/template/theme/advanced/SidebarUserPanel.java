			...
			new EnclosureContainer("userContainer")
				...
				.add(
					...
					ProfilePage.linkDescriptor()
						.link("profileLink")
						.add(
							new CoreLabel("fullName", BindingModel.of(BasicApplicationSession.get().getUserModel(), Bindings.user().fullName()))
								.hideIfEmpty()
						),
					new CoreLabel("username", BindingModel.of(BasicApplicationSession.get().getUserModel(), Bindings.user().username()))
						.hideIfEmpty(),
